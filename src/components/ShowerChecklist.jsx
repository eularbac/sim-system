...
/**
 * Checklist backed by public.shower_checklist, filtered by `tipo`
 * (Chá Bar | Chá de Panela | Chá de Lingerie | Chá de Casa Nova).
 */
export default function ShowerChecklist({ tipo }) {
  const { user } = useAuth()
  ...
  async function addItem(e) {
    e.preventDefault()
    if (!novo.trim()) return
    await supabase.from('shower_checklist').insert({ profile_id: user.id, tipo, item: novo.trim() })
    setNovo('')
    loadData()
  }
  ...
  async function toggle(id, feito) {
    ...
    await supabase.from('shower_checklist').update({ feito: !feito }).eq('id', id)
  }

  async function remove(id) {
    setItems((prev) => prev.filter((it) => it.id !== id))
    await supabase.from('shower_checklist').delete().eq('id', id)
  }
    ...
    <div>
      ...
      >
        <label className="flex-1 min-w-[200px]">
          <span className="font-body text-xs text-espresso-500 mb-1 block">Novo passo</span>
          <input
            value={novo}
            onChange={(e) => setNovo(e.target.value)}
            placeholder="Adicione algo específico do seu chá..."
      ...
      {loading ? (
        ...
        <p className="font-body text-espresso-400 italic">Nenhum passo ainda. Adicione o primeiro acima.</p>
      ) : (
